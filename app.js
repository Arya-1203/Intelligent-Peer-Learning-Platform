      ...
      <div class="dashboard-card">
        ...
        <div class="courses-list">
          ${enrolledCourses.slice(0, 2).map(course => `
            <div class="course-item" onclick="showCourseDetails(${course.id}); event.stopPropagation();">
      ...
      <div class="dashboard-card">
        ...
        <div class="courses-list">
          ${teachingCourses.slice(0, 2).map(course => `
            <div class="course-item" onclick="showCourseDetails(${course.id}); event.stopPropagation();" style="cursor: pointer;">
              <div class="course-title">${course.title}</div>
...
function joinStudyGroup(groupId) {
  ...
  if (group) {
    if (!currentUser.studyGroups) {
      currentUser.studyGroups = [];
    }
    if (!currentUser.studyGroups.includes(groupId)) {
      currentUser.studyGroups.push(groupId);
      group.members += 1;
...
function viewGroupDetails(groupId) {
  const group = APP_DATA.studyGroups.find(g => g.id === groupId);
  if (group) {
    document.getElementById('course-modal-title').textContent = group.name;
    const modalBody = document.getElementById('course-modal-body');
    modalBody.innerHTML = `
      <div class="course-details">
        <div class="course-info" style="margin-bottom: var(--space-24);">
          <div style="display: flex; align-items: center; gap: var(--space-16); margin-bottom: var(--space-16);">
            <div style="font-size: var(--font-size-4xl);">${group.avatar}</div>
            <div>
              <h3 style="margin: 0 0 var(--space-8) 0;">${group.name}</h3>
              <p style="color: var(--color-text-secondary); margin: 0;">${group.course}</p>
            </div>
          </div>
          <p><strong>Status:</strong> <span class="status status--${group.isActive ? 'success' : 'info'}">${group.isActive ? 'Active' : 'Inactive'}</span></p>
          <p><strong>Members:</strong> ${group.members}</p>
          <p><strong>Meeting Time:</strong> ${group.meetingTime}</p>
        </div>
        
        <div class="course-description" style="margin-bottom: var(--space-24);">
          <h4>Description</h4>
          <p>${group.description}</p>
        </div>
        
        <div style="display: flex; gap: var(--space-12);">
          <button class="btn btn--primary" onclick="joinStudyGroup(${groupId}); closeCourseModal();">Join Group</button>
          <button class="btn btn--outline" onclick="startGroupChat(${groupId})">Start Group Chat</button>
        </div>
      </div>
    `;
...
function createStudyGroup() {
  document.getElementById('course-modal-title').textContent = 'Create New Study Group';
  const modalBody = document.getElementById('course-modal-body');
  modalBody.innerHTML = `
    <form onsubmit="handleCreateStudyGroup(event)">
      ...
      <div class="form-group">
        ...
        <textarea class="form-control" id="group-description" rows="4" required></textarea>
      </div>
      <div class="form-group">
        <label class="form-label">Related Course</label>
        <select class="form-control" id="group-course" required>
          <option value="">Select a course</option>
          ${APP_DATA.courses.filter(c => currentUser.courses.includes(c.id)).map(c => 
      ...
      <div class="form-group">
        ...
        <input type="text" class="form-control" id="group-meeting-time" placeholder="e.g., Wednesdays 7:00 PM EST" required>
      </div>
      <div style="display: flex; gap: var(--space-12);">
...
function handleCreateStudyGroup(event) {
  event.preventDefault();
  const name = document.getElementById('group-name').value;
  const description = document.getElementById('group-description').value;
  const courseId = parseInt(document.getElementById('group-course').value);
  const meetingTime = document.getElementById('group-meeting-time').value;
  const course = APP_DATA.courses.find(c => c.id === courseId);
  
  const newGroup = {
    id: APP_DATA.studyGroups.length + 1,
    name: name,
  ...
  APP_DATA.studyGroups.push(newGroup);
  if (!currentUser.studyGroups) {
    currentUser.studyGroups = [];
  }
  currentUser.studyGroups.push(newGroup.id);
  localStorage.setItem('iplp_current_user', JSON.stringify(currentUser));
    ...
    <div class="course-card" onclick="showCourseDetails(${course.id})">
      ...
      <div class="course-body">
        ...
        ${enrolled ? `
          ...
          <button class="btn btn--primary btn--sm" onclick="continueLearning(${course.id}); event.stopPropagation();">Continue Learning</button>
        ` : `
...
function loadProgress() {
    ...
    <div class="progress-section">
      ...
      <div class="progress-details mt-24">
        ${APP_DATA.courses.filter(c => currentUser.courses.includes(c.id)).map(course => `
          <div class="progress-item card" style="padding: var(--space-20); margin-bottom: var(--space-16);">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--space-12);">
              <h4 style="margin: 0;">${course.title}</h4>
              <button class="btn btn--outline btn--sm" onclick="showCourseDetails(${course.id})">View Details</button>
            </div>
            ...
            <div style="display: flex; justify-content: space-between; align-items: center;">
              ...
              <span style="color: var(--color-text-secondary); font-size: var(--font-size-sm);">${course.modules.filter(m => m.completed).length}/${course.modules.length} modules completed</span>
            </div>
          </div>
        `).join('')}
      </div>
  ...
  setTimeout(() => {
    const detailedProgressCanvas = document.getElementById('detailed-progress-chart');
    if (detailedProgressCanvas) {
      const enrolledCourses = APP_DATA.courses.filter(c => currentUser.courses.includes(c.id));
      new Chart(detailedProgressCanvas, {
        type: 'bar',
        data: {
...
function loadAssignments() {
  ...
  let assignmentsHTML = `
    <div class="assignments-section">
      <div class="section-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--space-24);">
        <div>
          <h2>Assignments</h2>
          <p style="color: var(--color-text-secondary); margin-top: var(--space-8);">${currentUser.role === 'teacher' ? 'Manage and create assignments for your courses' : 'View and submit your assignments'}</p>
        </div>
        ${currentUser.role === 'teacher' ? `<button class="btn btn--primary" onclick="showCreateAssignmentModal()">Create Assignment</button>` : ''}
      </div>
      <div class="assignments-grid">
        ${assignments.length > 0 ? assignments.map(assignment => `
          <div class="assignment-card card" style="padding: var(--space-20);">
            <div style="margin-bottom: var(--space-12);">
              <h4 style="margin: 0 0 var(--space-8) 0;">${assignment.title}</h4>
              <p style="color: var(--color-text-secondary); font-size: var(--font-size-sm); margin: 0;">Course: ${assignment.courseTitle}</p>
            </div>
            <p style="margin: var(--space-8) 0;"><strong>Due:</strong> ${assignment.dueDate}</p>
            <div class="assignment-status" style="margin-bottom: var(--space-12);">
              ${assignment.submitted ? 
                `<span class="status status--success">Submitted</span>` :
                `<span class="status status--warning">Pending</span>`
              }
            </div>
            ${assignment.grade ? `<p style="margin: var(--space-8) 0;"><strong>Grade:</strong> ${assignment.grade}</p>` : ''}
            ${!assignment.submitted && currentUser.role === 'student' ? 
              `<button class="btn btn--primary btn--sm" onclick="submitAssignment(${assignment.id}, ${assignment.courseId})">Submit Assignment</button>` : 
              ''
            }
            ${currentUser.role === 'teacher' ? 
              `<button class="btn btn--outline btn--sm" onclick="viewAssignmentSubmissions(${assignment.id}, ${assignment.courseId})">View Submissions</button>` : 
              ''
            }
          </div>
        `).join('') : `
          <div class="card" style="padding: var(--space-32); text-align: center;">
            <p style="color: var(--color-text-secondary);">No assignments found.</p>
          </div>
        `}
      </div>
    </div>
  ...
  pageContent.innerHTML = assignmentsHTML;
}
...
function sendConnectionRequest(peerId) {
  ...
  if (peer) {
    ...
    if (!currentUser.connections.includes(peerId)) {
      currentUser.connections.push(peerId);
      localStorage.setItem('iplp_current_user', JSON.stringify(currentUser));
      
      // Add notification for the peer (simulated)
      const notification = {
        id: APP_DATA.notifications.length + 1,
      ...
      APP_DATA.notifications.push(notification);
      
      showToast(`Connection request sent to ${peer.name}!`, 'success');
    } else {
      showToast('You are already connected with this peer!', 'info');
    }
  }
}
...
function continueLearning(courseId) {
  ...
  if (course) {
    ...
    if (nextModule) {
      ...
      setTimeout(() => {
        const modalBody = document.getElementById('course-modal-body');
        const moduleSection = modalBody.querySelector('.course-modules');
        if (moduleSection) {
          const moduleItems = moduleSection.querySelectorAll('.module-item');
          moduleItems.forEach((item, index) => {
            if (course.modules[index].id === nextModule.id) {
        ...
        showToast(`Continue with: ${nextModule.title}`, 'info');
      }, 300);
    } else {
      showCourseDetails(courseId);
      showToast('Congratulations! You have completed all modules!', 'success');
    }
  }
}

function showCreateAssignmentModal() {
  const teachingCourses = APP_DATA.courses.filter(c => 
    currentUser.courses_teaching?.includes(c.id)
  );
  
  if (teachingCourses.length === 0) {
    showToast('You need to have courses to create assignments!', 'warning');
    return;
  }
  
  document.getElementById('course-modal-title').textContent = 'Create New Assignment';
  const modalBody = document.getElementById('course-modal-body');
  modalBody.innerHTML = `
    <form onsubmit="handleCreateAssignment(event)">
      ...
      <div class="form-group">
        ...
        <select class="form-control" id="assignment-course" required>
          <option value="">Select a course</option>
      ...
      <div class="form-group">
        ...
        <input type="number" class="form-control" id="assignment-points" min="1" max="100" value="100" required>
      </div>
      <div style="display: flex; gap: var(--space-12);">
...
function handleCreateAssignment(event) {
  ...
  if (course) {
    ...
    const newAssignment = {
      id: course.assignments.length > 0 ? Math.max(...course.assignments.map(a => a.id)) + 1 : 1,
      title: title,
      dueDate: dueDate,
      description: description,
      points: points,
    ...
    // Add notification for enrolled students
    const enrolledStudents = APP_DATA.users.filter(u => 
      u.role === 'student' && u.courses && u.courses.includes(courseId)
    );
    enrolledStudents.forEach(student => {
      const notification = {
        id: APP_DATA.notifications.length + 1,
        title: 'New Assignment',
        message: `${title} has been added to ${course.title}`,
        time: 'Just now',
        type: 'warning',
        read: false
      };
...
function submitAssignment(assignmentId, courseId) {
  const course = APP_DATA.courses.find(c => c.id === courseId);
...
function viewAssignmentSubmissions(assignmentId, courseId) {
  const course = APP_DATA.courses.find(c => c.id === courseId);
  if (course && course.assignments) {
    const assignment = course.assignments.find(a => a.id === assignmentId);
    if (assignment) {
      ...
      document.getElementById('course-modal-title').textContent = `Submissions: ${assignment.title}`;
      const modalBody = document.getElementById('course-modal-body');
      modalBody.innerHTML = `
        <div class="course-details">
          <div class="course-info" style="margin-bottom: var(--space-24);">
          ...
          <h4 style="margin-bottom: var(--space-16);">Student Submissions</h4>
          <div class="submissions-list">
            ${enrolledStudents.map(student => {
              const submitted = assignment.submitted;
              return `
                <div class="submission-item card" style="padding: var(--space-16); margin-bottom: var(--space-12);">
                  <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                      <div style="font-weight: var(--font-weight-medium);">${student.name}</div>
                      <div style="color: var(--color-text-secondary); font-size: var(--font-size-sm);">${student.email}</div>
                    </div>
                    <div>
                      ${submitted ? 
                        `<span class="status status--success">Submitted</span>` :
                        `<span class="status status--warning">Not Submitted</span>`
                      }
                    </div>
                  </div>
                  ${submitted ? `
                    <div style="margin-top: var(--space-12);">
                      <button class="btn btn--primary btn--sm" onclick="gradeAssignment(${assignmentId}, ${courseId}, ${student.id})">Grade Assignment</button>
                    </div>
                  ` : ''}
                </div>
              `;
...
function gradeAssignment(assignmentId, courseId, studentId) {
  const grade = prompt('Enter grade (A, B, C, D, F, or numeric score):');
  if (grade) {
    const course = APP_DATA.courses.find(c => c.id === courseId);
    if (course && course.assignments) {
      const assignment = course.assignments.find(a => a.id === assignmentId);
      if (assignment) {
        ...
        viewAssignmentSubmissions(assignmentId, courseId);
      }
    }
  }
}
...
function startGroupChat(groupId) {
  const group = APP_DATA.studyGroups.find(g => g.id === groupId);
  if (group) {
    showToast(`Opening group chat for ${group.name}...`, 'info');
    // In a real app, this would open a group chat interface
    closeCourseModal();
  }
}